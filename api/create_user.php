 ...
 * Executa os comandos SQL para criar o usuário do banco
 */
...
class CreateDatabaseUser {
    ...
    /**
     * Solicita credenciais do root ao usuário
     */
    private function promptRootCredentials() {
        if (php_sapi_name() !== 'cli') {
            return;
        }
        
        echo "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
        echo "Credenciais do MySQL ROOT\n";
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
        
        echo "Usuário ROOT MySQL [padrão: root]: ";
        $input = trim(fgets(STDIN));
        if ($input) {
            $this->rootUser = $input;
        }
        
        echo "Senha ROOT MySQL [deixe em branco se não houver]: ";
        $input = trim(fgets(STDIN));
        if ($input) {
            $this->rootPassword = $input;
        }
    ...
    /**
     * Lê as credenciais do arquivo de configuração gerado
     */
    private function getCredentialsFromConfig() {
        ...
        // Ler arquivo e extrair credenciais
    ...
    private function connectAsRoot() {
        ...
        if ($conn->connect_error) {
            throw new Exception("Erro ao conectar como root: " . $conn->connect_error);
        }
    ...
    public function createUser() {
        try {
            ...
            // Solicitar credenciais root se estiver no CLI
            if (php_sapi_name() === 'cli') {
                $this->promptRootCredentials();
            }
            ...
            if (!$conn->query("CREATE DATABASE IF NOT EXISTS plataforma_x")) {
                throw new Exception("Erro ao criar banco: " . $conn->error);
            }
            echo "   ✓ Banco de dados criado\n";
            
            // Conceder privilégios
            $sqlGrants = "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE ON plataforma_x.* TO '$user'@'$this->host'";
...
if (php_sapi_name() === 'cli' || (isset($_GET['action']) && $_GET['action'] === 'create_user')) {
